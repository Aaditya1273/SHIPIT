...
export class HomeScene extends Phaser.Scene {
  ...
  async create() {
    ...
    try {
      ...
      if (!this.gameData.villagers) {
        console.warn("No villagers returned from startNewGame, defaulting to empty array.");
        this.gameData.villagers = [];
      }
      ...
      // Inventory is managed via localStorage/backend in 0G flow
      ...
      currentGameItems = ALL_POSSIBLE_ITEMS.slice(0, 4);
      ...
      console.error("Critical Error in HomeScene create:", err);
      // Attempt to clear loading screen even if error occurred so user isn't stuck
      if (overlay) overlay.destroy();
      if (loadingPanel) loadingPanel.destroy();
      if (gameTitle) gameTitle.destroy();
      if (loadingSubtitle) loadingSubtitle.destroy();
      if (progressBox) progressBox.destroy();
      if (progressBar) progressBar.destroy();
      if (percentText) percentText.destroy();
      if (statusText) statusText.destroy();
      if (loadingDots) loadingDots.destroy();
    }
    ...
    // Zoom tween moved to startIntroSequence
    ...
    console.log("Skip Tutorial Flag:", skipTutorial);
  ...
  showTutorial() {
    ...
    // Hide HUD while tutorial is open
    this.scene.setVisible(false, 'UIScene');

    const width = this.cameras.main.width;
    const height = this.cameras.main.height;

    const tutorialContainer = this.add.container(0, 0).setDepth(2000).setScrollFactor(0);
    ...
    panel.strokeRoundedRect(panelX + 4, panelY + 4, panelWidth - 8, panelHeight - 8, 20);

    tutorialContainer.add(panel);

    // Title
    const titleText = this.add.text(width / 2, panelY + 40, "How to Play", {
      fontFamily: "Georgia, serif",
      fontSize: "36px",
      color: "#d4af37",
      stroke: "#000000",
    ...
    const steps = [
      "1. Explore the village using WASD or Arrow Keys.",
      "2. Talk to villagers by pressing ENTER near them. Some have quests!",
      "3. Press 'M' to mint items at specific locations (e.g., Lake, Forest).",
      "4. Find your lost friends and uncover the mystery of the mist.",
    ...
    const checkboxY = panelY + panelHeight - 140;
    const checkboxX = width / 2 - 120; // Center roughly

    const checkboxBox = this.add.rectangle(checkboxX, checkboxY, 24, 24).setStrokeStyle(2, 0xd4af37).setOrigin(0.5);
    ...
    const checkboxZone = this.add.zone(checkboxX, checkboxY, 300, 40).setOrigin(0, 0.5).setInteractive({ useHandCursor: true });

    checkboxZone.on('pointerdown', () => {
      dontShowAgain = !dontShowAgain;
      checkboxCheck.setVisible(dontShowAgain);
      checkboxBox.setFillStyle(dontShowAgain ? 0x2c2c54 : 0x000000, 0); // Visual feedback
    });

    tutorialContainer.add([checkboxBox, checkboxCheck, checkboxLabel, checkboxZone]);
    ...
    btnBg.fillRoundedRect(-100, -25, 200, 50, 15);
    ...
    tutorialContainer.add(button);

    // Animate in
    tutorialContainer.setAlpha(0).setScale(0.9);
    this.tweens.add({
      targets: tutorialContainer,
      alpha: 1,
      scale: 1,
